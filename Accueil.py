...
st.set_page_config(
    page_title="Tuteur IA Mathématiques (SystSeme Marocain)",
    layout="wide",
...
# REMOVED: if 'should_rerun' not in st.session_state: st.session_state.should_rerun = False 
...
def call_gemini_api(prompt, image_part=None):
    ...
    # Remplace les accents par leur version sans accent pour assurer une compatibilite ASCII de base
    def safe_level(level):
        return level.replace('(', '').replace(')', '').replace('è', 'e').replace('é', 'e').replace('É', 'E').replace('à', 'a').replace('À', 'A').replace('ô', 'o').replace('û', 'u')

    school_level_safe = safe_level(school_level_raw)
    ...
    # NOTE: L'instruction finale utilise des termes generaux (Markdown, LaTeX) pour rester compatible
    final_system_prompt = f"{system_prompt_base} {lang_instruction} {style_instruction} Utilise le format Markdown pour organiser ta reponse, et assure-toi que les formules mathematiques sont formatees en LaTeX."
...
def load_user_session(email, save_cookie=False):
    ...
    if user_data:
        ...
        # RERUN DIRECTEMENT: st.session_state.should_rerun = True 
        st.rerun() # <<< MODIFICATION: Remplacer experimental_rerun par rerun
        return True
    return False
...
def auth_ui():
    ...
    with col2:
        with st.form("register_form"):
            ...
            st.subheader("Vos Parametres par Defaut")
...
def update_profile_settings():
    ...
    st.rerun() # <<< MODIFICATION: Recharger apres la mise a jour pour voir les changements
...
def profile_ui():
    ...
    # Utiliser st.experimental_get_query_params pour obtenir la base url dans Streamlit
    base_url = st.get_option('server.baseUrl') if st.get_option('server.baseUrl') else ""
    referral_link = f"{base_url}?{REFERRAL_PARAM}={st.session_state.user_email}"
    st.sidebar.caption("Partagez ce lien لكسب 10 أسئلة إضافية!")
...
def main_app_ui():
    ...
    st.markdown("""
    **Bienvenue!** انا مساعدك الذكي المتخصص، جاهز لمساعدتك في حل مشاكلك الرياضية. يمكنك طرح سؤال أو **تحميل صورة** للتمرين.
    """)

    uploaded_file = st.file_uploader(
        "اختياري: قم بتحميل صورة لتمرين رياضي (JPG أو PNG). الحد الأقصى 4 ميغا بايت.",
        type=["png", "jpg", "jpeg"],
        key="image_uploader"
    )
    ...
    user_prompt = st.text_area(
        "أضف سؤالك أو تعليماتك هنا (حتى لو قمت بتحميل صورة).",
        height=100,
        key="prompt_input"
    )

    if st.button("توليد الإجابة الرياضية", use_container_width=True, type="primary"):
        if not user_prompt and not uploaded_file:
            st.warning("الرجاء إدخال سؤال أو تحميل صورة لبدء التوليد.")
        else:
            if uploaded_file and uploaded_file.size > 4 * 1024 * 1024:
                st.error("الصورة كبيرة جداً (الحد الأقصى 4 ميغا بايت).")
            else:
                with st.spinner("تحليل المشكلة وتوليد الإجابة..."):
                ...
                st.subheader(f"✅ الإجابة المولدة ({st.session_state.user_lang})") 
                
                # Utiliser st.markdown pour interpreter le Markdown/LaTeX de la reponse
                st.markdown(response_text)
                
                if sources:
                    st.subheader("🌐 المصادر المستخدمة (Google Search Grounding)")
                ...
                if not st.session_state.is_unlimited:
                    ...
                    if remaining > 0:
                         st.info(f"الأسئلة المتبقية اليوم: **{remaining}**")
                    else:
                        st.error("تم الوصول إلى الحد اليومي.")
...
def main():
    ...
    # 2. Ancien point de relance supprime (st.session_state.should_rerun n'est plus necessaire)
...
if __name__ == '__main__':
    # Nous n'avons plus besoin de st.experimental_rerun() ici
    main()
